...
static XImage *
MakeXImage                      (Display *, int, int);

static int
XRotPaintAlignedString          (Display *,
                                 XFontStruct *,
                                 float,
                                 Drawable,
                                 GC,
...
static int
XRotDrawHorizontalString        (Display *,
                                 XFontStruct *,
                                 Drawable,
                                 GC,
                                 int,
...
static void
XRotAddToLinkedList             (Display *, RotatedTextItem *);

static void
XRotFreeTextItem                (Display *, RotatedTextItem *);

static XImage *
XRotMagnifyImage                (Display *, XImage *);
...
static char *
my_strtok       (register char *str1, register char *str2)
{
  register char *p;
  register char *a, *b;         /* first, last (used) */
  static char   *a_, *b_;       /* first, last (remembered) */
  ...
  *a = 0;       /* turn delimitor into null */
  p = a_;       /* return value */
  a_ = a+1;     /* advance to following char for next call */
...
int
XRotDrawImageString     (Display        *dpy,
                         XFontStruct    *font,
                         float          angle,
                         Drawable       drawable,
                         GC             gc,
...
int
XRotDrawAlignedString   (Display        *dpy,
                         XFontStruct    *font,
                         float          angle,
                         Drawable       drawable,
                         GC             gc,
...
int
XRotDrawAlignedImageString      (Display        *dpy,
                                 XFontStruct    *font,
                                 float          angle,
                                 Drawable       drawable,
                                 GC             gc,
...
static int
XRotPaintAlignedString  (Display        *dpy,
                         XFontStruct    *font,
                         float          angle,
                         Drawable       drawable,
                         GC             gc,
                         ...
                         int            bg)
{
    ...
    if(bg) {
        ...
        for(i=0; i<4*item->nl; i++) {
            xpoints[i].x=(short)((float)x + ( (item->corners_x[i]-hot_x)*cos_angle + 
                                      (item->corners_y[i]+hot_y)*sin_angle));
            xpoints[i].y=(short)((float)y + (-(item->corners_x[i]-hot_x)*sin_angle + 
...
static int
XRotDrawHorizontalString        (Display        *dpy,
                                 XFontStruct    *font,
                                 Drawable       drawable,
                                 GC             gc,
                                 int            x,
...
static RotatedTextItem *
XRotRetrieveFromCache   (Display        *dpy,
                         XFontStruct    *font,
                         float          angle,
                         char           *text,
                         int            align)
...
static RotatedTextItem *
XRotCreateTextItem      (Display        *dpy,
                         XFontStruct    *font,
                         float          angle,
                         char           *text,
                         int            align)
	  ...
	  xinc=0;
        ...
        xl=(float)((float)item->cols_out/2+
            (dj-(float)item->rows_in/(2*cos_angle))/
                tan(angle)-2);
        xr=(float)((float)item->cols_out/2+
            (dj+(float)item->rows_in/(2*cos_angle))/
                tan(angle)+2);
    ...
    for(j2=0; j2<rows_out; j2++) {
        ...
        for(i2=0; i2<cols_out; i2++) {
            ...
            if(i==cols_in-1 && j!=rows_in-1) {
                ...
                z1=(float)((ximage->data[j*byte_width_in+i/8] & 128>>(i%8))>0);
                z2=z1;
                z3=(float)((ximage->data[(j+1)*byte_width_in+i/8] &
			     128>>(i%8))>0);
                z4=z3;
            }
            /* top edge */
                ...
                z1=(float)((ximage->data[j*byte_width_in+i/8] & 128>>(i%8))>0);
                z2=(float)((ximage->data[j*byte_width_in+(i+1)/8] &
			     128>>((i+1)%8))>0);
                z3=z2;
                z4=z1;
            }
            ...
            /* somewhere `safe' */
                ...
                z2=(float)((ximage->data[j*byte_width_in+(i+1)/8] &
		    128>>((i+1)%8))>0);
                z3=(float)((ximage->data[(j+1)*byte_width_in+(i+1)/8] &
                    128>>((i+1)%8))>0);
                z4=(float)((ximage->data[(j+1)*byte_width_in+i/8] &
		    128>>(i%8))>0);
            }
                 ...
                 int            align)
{
    ...
    for (i = 0; i < nl; i++)
    {
      ...
      {
        XTextExtents
          (font, str3, strlen(str3), &dir, &asc, &desc, &overall);
        if (overall.rbearing > max_width)
          max_width=overall.rbearing;

        /* if this is the first line, record the amount of excess
         * space above the heightest point.  Similarly, record the
         * excess below the lowest point for the las line */
        if (i == 0) excess_height += asc - overall.ascent;
        if (i == (nl - 1)) excess_height += desc - overall.descent;
      }
      str3 = my_strtok ((char *)NULL, str2);
    }
    ...
    cols_in=max_width;
#if 1
    rows_in=nl*height - excess_height;
#else
    rows_in=nl*height;
#endif
    ...
    for(i=0; i<5; i++) {
        xp_out[i].x=(short)((float)x + ( ((float)xp_in[i].x-hot_x)*cos_angle +
                                 ((float)xp_in[i].y+hot_y)*sin_angle));
        xp_out[i].y=(short)((float)y + (-((float)xp_in[i].x-hot_x)*sin_angle +
