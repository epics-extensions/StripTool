...
main (int argc, char *argv[])
{
  ...
  if (!(strip = Strip_init ("StripTool", &argc, argv)))
    {
...
static int	request_connect		(StripCurve curve, void *blah)
{
  int ret_val = 0;
...
static int	request_disconnect	(StripCurve curve, void *blah)
{
  int ret_val = 0;
...
static double	get_cpu_usage		(void *blah)
{
  ...
  static clock_t		clock_a = -1;
  static clock_t		clock_b = -1;
